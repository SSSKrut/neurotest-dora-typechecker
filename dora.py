...
def extract_imports(node: ast.AST) -> dict:  
    """  
    Extract import aliases to resolve fully qualified names.  
    Returns a dictionary mapping aliases to module names.  
    """  
    aliases = {}  
    for child in ast.iter_child_nodes(node):  
        if isinstance(child, ast.Import):  
            for alias in child.names:  
                name = alias.name  # The actual module  
...
def search_file(file_path: str, target_type: Optional[str]) -> List[Tuple[int, int, str, str, str]]:  
    """  
    Search a single file for type annotations or expressions.  
    Returns a list of tuples:  
    (line_no, col_offset, type_str, expr_type, source_line)  
    """  
    matches = []  
    try:  
        ...
        for node, type_str in annotations_and_exprs:  
            ...
            # Reconstruct fully qualified type name if possible  
            fq_type_str = type_str  

            if target_type:  
                if fq_type_str != target_type and not fq_type_str.endswith(target_type):  
                    continue  # Skip if doesn't match target type  

            expr_type = get_expr_type(node)  

            # Extract the expression string from the source line  
            ...
            matches.append((line_no, col_offset, fq_type_str, expr_type, source_line, expr_str))  
...
def main():  
    ...
    for file in py_files:  
        ...
        for match in matches:  
            line_no, col_offset, type_str, expr_type, source_line, expr_str = match  
            print(f"{file}:{line_no}:{col_offset}")  
            # Prepare the expression display with color  
            colored_expr = colorize(expr_str, COLOR_CYAN)  
            print(f"    {colored_expr} ({expr_type})")  
            print("    v")  
            # Highlight the expression in the source line  
